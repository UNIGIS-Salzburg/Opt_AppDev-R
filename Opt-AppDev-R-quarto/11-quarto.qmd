...
According to the [Quarto Website](https://quarto.org/){target="_blank"}, *Quarto is an open-source scientific and technical publishing system*, and it is already available on your machines because you installed it together with RStudio.
...
In our example the `dev` branch is two commits ahead of the `main` branch. You may [open a pull request](https://docs.github.com/en/get-started/quickstart/hello-world#opening-a-pull-request){target="_blank"} to merge changes from `dev` to `main` branch.
