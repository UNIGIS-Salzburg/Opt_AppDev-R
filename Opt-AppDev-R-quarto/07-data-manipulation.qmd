...
In the example above, we use the pipe operator `%>%`, a key feature of `magrittr` and `tidyverse` syntax. The operator links a sequence of analysis steps, passing `flights_from_nyc` through `count()` and then `kable()`. This makes the code simpler and more readable. 
...
::: {#fig-vidpip}
<iframe src="https://vimeo.com/1074652888/be7c285395?share=copy" allow="autoplay; fullscreen; picture-in-picture" allowfullscreen>
