...
The code below, loads the data frame `flights` from the library `nycflights13` into the variable `flights_from_nyc`. We will use this data frame in this lesson. 
...
The operator `::` is used to indicate that the data frame `flights` is situated within the library `nycflights13`. This helps avoiding ambiguities in the case functions from different loaded libraries have identical names. 
...
Alternatively, you may inspect the data frame `flights_from_nyc` by writing it to the console.

```

## Data manipulation

The library `dplyr` provides a number of functions to investigate basic characteristics of inputs. For instance, the function `count()` can be used to count the number of rows of a data frame. The code below provides `flights_from_nyc` as input to the function. 
...
```{block2, type = 'rmdtip'}
The pipe operator allows us to link a sequence of analysis steps. In the example above the data frame `flights_from_nyc` is passed into function `count()` and the output is passed into function `kable()` to render the result_df
...
The function `arrange()` sorts a [tibble](https://tibble.tidyverse.org/){target="_blank"} by ascending order of the values in the specified column. If the operator `-` is specified before the column name, the descending order is used. The code below would produce a table showing all the rows when ordered by descending order of air time.
...
```{block2, type = 'rmdtip'}
Note that data frames in the code above are converted to so called [tibbles](https://tibble.tidyverse.org/){target="_blank"}, which is another tabular format. This step is needed as the function `kable()` takes `tibbles` as an input. 
```
