...
The most common geometry types are points, lines and polygons and their respective "multi" versions. Let's explore each of these in detail.
...
Sf-object are similar in structure as data frames. However, in contrast to data frames sf-objects have an additional geometry column. The sf-object `point_sf` contains a single point geometry and no fields:
...
In principle sf-objects can be treated like data frames. Accordingly, data frame syntax is used to assign fields to geometries: 
...
Plotting the sf-object metadata in the console reveals that `Geometry type` is `Multipoint` and that no coordinate coordinate reference system has been defined (CRS: NA):
...
Now we transform the sf-object to CRS NAD27 with function `st_transform`:

```{r}

# Assign NAD27 as CRS
point_sf_multi_transform <- sf::st_transform(point_sf_multi, 4267)
...
plot(sf::st_geometry(point_sf_multi_transform), main = "EPSG: 4267")
