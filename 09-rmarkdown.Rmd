...
It his highly recommended to keep work in progress separate from the `main` branch by working in a separate developer branch. Later, we will merge our changes from `dev` into the `main` branch.
...
Moreover, hash signs indicate headings. The number of hash signs you use corresponds to the heading level:  
...
Once you have enabled BibTeX export in your browser, a new link appears (see Fig. \@ref(fig:L09-biboption)).
...
To compile your R Markdown document, `knit` as HTML, PDF or Word. The `rmarkdown` library renders references as indirect citations (*without squared brackets*) and direct citations (*with squared brackets*)  and inserts a bibliography (see Fig. \@ref(fig:L09-pdfknit)).
...
[Here]("data/ReferencesInRMarkdown.zip"){target="_blank"} you can find the full worked example for download.
