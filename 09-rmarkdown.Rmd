...
Ordered Lists are made by create numbers followed by a period. The first should start with the number one.
...
Unordered Lists are easily made by making one *, - or +. Leave a blank line before the first element and you have a list:
...
Now you might ask yourself if we want to use a \# hash sign, or a double underscore \_italic_ for example. But not for a heading or italic style, but in our sentence. How can we tell Markdown to **ignore these special characters**? This works with an **\(\) backslash**. Simply, put the backslash before your special character. The backslash sign then gets ignored and you will see the specific sign. 

If you need to write a scientific paper and/or need to proof something. >ou will also oftentimes need **mathematical formula**.These are absolutely based on LaTeX - you will find more formulas [here](https://en.wikibooks.org/wiki/LaTeX/Mathematics){target="_blank"}. Though, in R Markdown, you can call these by using the "\$" icon before and after writing, this will create an **inline equation**. A Double "\$$" icon means its a **display equation**. They come in very handy.
...
Other R Markdown compile formats that are described in this book are Notebooks and Presentations. As mentioned in the Python example, RMarkdown also supports other languages such as Python, C++ and SQL. Moreover, R Markdown is customizable and can be used to design more complex documents with extensions like [BookDown](https://bookdown.org/){target="_blank"} or [ThesisDown](https://github.com/ismayc/thesisdown){target="_blank"}.
