...
In this lession I will introduce a series of more complex data types that are built on top of the already discussed simple data types 'numeric', 'character' (string) and 'logical' (see [Lesson 1](#intro) 'Simple data types').

```{block2, type = 'rmdtip'}
In this lession, you will get to know the following complex data types:
...
Elements of a vector (data frame column) must be of the same type (charater, logical or numeric). In the example above, the column 'EmployeeName' contains only character strings, the column 'Age' contains only numeric etc.
...
employees <- data.frame(
  EmployeeName = c(<span>&#34;</span>Maria<span>&#36;</span>, "Pete", "Sarah"),
  Age = c(47, 34, 32),
  Role = c("Professor", "Researcher", "Researcher"))  
