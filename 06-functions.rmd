...
```{block2, type = 'rmdtip'}
In previous lessons, we've used various functions (like `cat()`, `print()`) without delving deeply into their mechanics. This lesson aims to demystify functions, guiding you through creating and understanding custom functions. We'll also explore the concepts of global and local variable scopes.
```
...
Invoke this function by specifying its **identifier** and the necessary parameter(s). For instance, in our example `add_one(3)` returns `4`.
...
In both examples, the function `add_one` returns 4. The explicit use of `return()` in the second example provides clarity about the intended output of the function.
...
Sometimes, it's necessary to exit a function early, such as in error handling:
...
Default parameters can enhance a function's flexibility. Modify the function parameters as `(height, width = 3)`. With only one input, the function should return `YOUR INPUT * 3`. Specifying both parameters overrides the default `width`.
...
Retrieve these values using list indices `[[1]]` and `[[2]]`, or by simply their names:
...
The function takes a single numeric value as an input, defines a variable named `result` that is equal to '1' and then creates a loop over all the numbers from 1 (variable `result`) to the `input_value`. In the loop, the current value of `result` is multiplied by the value of the iterator `i`.  
