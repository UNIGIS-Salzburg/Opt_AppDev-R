...
In the code example below, the first argument in `ggplot2` is the [data](data/Aberdeen City.csv){target="_blank"} input (a data frame). In the second argument, we map the data onto the x-axis and y-axis with the aesthetics argument `aes()`.

```{r, echo=TRUE, message=FALSE, warning=FALSE, fig.height = 4}

library(ggplot2)

aberdeen_2011OAC <- read_csv("data/Aberdeen_City.csv")
...
`ggplot2` plots are objects which means that you can assign them. If we would display the plot now then we wouldn’t see anything yet because we have not specified the geometric element yet.
