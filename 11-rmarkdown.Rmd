...
::: {.rmdtip}
To automatically update the date in your document based on your system time, insert the following in the metadata section:

`` date: "`x format(Sys.time(), '%d %B, %Y')`"``

This outputs the current date based on your system's time zone in a human-readable format.

**Note:** Replace `x` with `r` to indicate an R code block that will be evaluated.
:::
