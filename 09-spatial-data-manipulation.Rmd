...
sf::st_distance(farms, invekos.sub) %>%
  head(1)

```

Function `st_distance` returns the shortest distance matrix between geometries. The function `head` is used to limit displayed outputs to the first matrix, which contains the distance between farm 1 and the centroid of 100 parcels in meters.
...
refer to igraph library for network analysis: 
https://r.igraph.org/
