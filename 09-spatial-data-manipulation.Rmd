...
In the previous lesson you have learned to get access to data on a server via Rest API. Recently, the Open Geospatial Consortium OGC has transferred the Rest API paradigm to geospatial fields of application. Meanwhile various OGC API standards have been implemented to provide and use geospatial data on the web. An overview of the current status of implementation can be found [here](https://ogcapi.ogc.org/#standards){target="_blank"}.

```{block2, type = 'rmdtip'}

In this lesson, we will use online data resources in a workflow that involves data cleaning, spatial queries and analyses.

In addition, you will learn to carry out basic raster manipulation operations on a sample raster dataset.
...
The resampling process can be illustrated based on the terra sample dataset:
...
The terra resampling function takes a target raster as an input. As we do not have a target raster available, we simply copy the original raster and assign a new raster resolution value to it:
...
The new values of raster `r_resampled` are calculated by method `bilinear`. Other methods are `nearest` or `cubic`. A discussion on what interpolation method to use for resampling raster data can be found [here](https://gis.stackexchange.com/questions/2587/deciding-what-interpolation-method-to-use-for-resampling-raster-data){target="_blank"}. 
...
You can crop a `SpatRaster Object` with a `SpatExtent`, or with another object from which an extent can be obtained.

To extract the extent of a `SpatRaster`, we can use the function `ext`:
...
The extent of object `r` is returned in the format <xmin, xmax, ymin, ymax>. Once we know the extent of the original object, the cropping extent can be specified by function `ext` in function `crop`. The `SpatExtent` named `crop_ext` is then used as an input to function `crop`, which a subset of `r` as `SpatRaster Object`:
...
You may have noticed that the extent in y-direction of the returned subset is larger than specified in the code above. This is because only entire cells can be selected. To circumvent this problem one can either change the cropping extent (e.g. 180000, 180200, 331000, 331200) or modify the spatial resolution of `r`.

### Raster algebra

Many generic function are implemented in `terra` that enable simple algebraic operations. This includes normal algebraic operators such as `+, -, *, /`, logical operators such as `<, <=, >, ==, !` and functions such as `abs`, `round` and others (see [here](https://rspatial.org/pkg/4-algebra.html){target="_blank"} to find a comprehensive list).

For instance, raster operators can be used to modify values of cells:

```{r, echo=TRUE, message=FALSE, warning = FALSE}

# add value 10 to every cell
s <- r + 10
# take the square root of values
s <- sqrt(r)
# return logical raster
s1 <- s == 15
...
The first example `s <- r + 10` illustrates that `SpatRaster Object` can be mixed with numbers, as long as the first argument is a `SpatRaster Object`. Alternatively, you can also use algebraic operators with multiple `SpatRaster Objects`. However, these must have the same resolution and origin. Typically these objects will also have the same extent, but if they do not, the returned object covers the spatial intersection of the objects used. 

Moreover, until recently also replacement function can be used: 
...
In the example above `value==255` is replaced by a value 2550.

A few more useful raster algebra examples can be found in the [terra materials](https://rspatial.org/pkg/4-algebra.html){target="_blank"}. A more comprehensive overview of available vector and raster operations is given in [Chaper 5 - Geometry operations](https://bookdown.org/robinlovelace/geocompr/geometric-operations.html){target="_blank"} of the E-Book [Geocomputation with R](https://bookdown.org/robinlovelace/geocompr/){target="_blank"}.

> Note: At the time of writing this module, Geocomputation with R uses package `raster` which is planned being replaced by `terra`.   
