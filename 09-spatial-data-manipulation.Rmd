...
As a next step, we can calculate geometric relations between farms and parcels. To illustrate the concept of binary geometric operations, parcels are allocated among farms based on euclidean distance. 

First the euclidean distance between farms and parcels needs to be calculated:
...
This index is extracted by function `which`. `which` is a base R function that returns the index of elements that fulfill a condition. In the example above, the index of the minimum value (see function `min`) in vector `dist_m[, 1]` is returned. As a result, the code returns index `1`; i.e. parcel 1 is closest to farm 1.

The demonstrated procedure for detecting closest farms can be executed for ever parcel in a for-loop. The number of the closest farm is appended to a vector `closest`. The vector is appended as a new column to sf-object `invekos.sub`. And `invekos.sub` is plotted together with `farms`:
...
Other geometrical operations are designed to return new geometries....for instance; See geometrical operations:
https://r-spatial.github.io/sf/articles/sf3.html#geometrical-operations

Union, buffer, intersect etc.

refer to igraph and Luc's library sfnetworks for network analysis: 
https://r.igraph.org/
