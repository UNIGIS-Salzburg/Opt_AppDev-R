...
The following code returns a subset of sf-object `invekos` (2 features with 3 non-spatial attributes and geometries):
...
The next line creates a basic map in sf by means of plot():
...
By default this creates a multi-panel plot, one sub-plot for each attribute of the object. To constrain the number of sub-plots, use `plot(invekos[<index number>])` or `plot(invekos[<index number start>:<index number end>])`. e.g.:
