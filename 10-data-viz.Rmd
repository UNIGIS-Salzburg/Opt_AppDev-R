...
Consider the example below where the variables 'Gdp per capita' and 'Life Expectancy' are aligned along the x and y axes, respectively, while 'national population' and 'world regions' are visually differentiated through size and color.

```{r L07-colsize, echo=FALSE, fig.cap="Visual variables color and size in a `ggplot2` graph", out.width='90%', fig.asp=.75, fig.align='center'}
...
```{block2, type = 'rmdtip'}
**Further Reading:** For a foundational understanding of the Grammar of Graphics as implemented in `ggplot2`, refer to Hadley Wickham's insightful [article](https://byrneslab.net/classes/biol607/readings/wickham_layered-grammar.pdf){target="_blank"}.
```
...
```{block2, type = 'rmdtip'}
**Note**: Transforming data between wide and long formats is often achieved using functions like `pivot_longer()` and `pivot_wider()` from the [`tidyr`](https://tidyr.tidyverse.org/reference/index.html){target="_blank"} package, which is in included in the Tidyverse.
```
